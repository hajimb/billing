...
class Customer extends REST_Controller {
    ...
    public function save_post(){
        ...
        // $this->form_validation->set_rules('doa', 'Date Of Anniversary', 'required|trim');
        ...
        if ($this->form_validation->run()) {
            ...
            $cusId = $this->post('id');
            $customer['c_name']    = trim($this->input->post('name'));
            $customer['email']     = trim($this->input->post('email'));
            $customer['mobile']    = trim($this->input->post('mobile'));
            $customer['dob']       = trim($this->input->post('dob'));
            $customer['doa']       = trim($this->input->post('doa'));
            $customer['address']   = trim($this->input->post('address'));
            $result                = $this->Customer_model->save($customer, $cusId);
            $this->response([
                'validate' => TRUE,
